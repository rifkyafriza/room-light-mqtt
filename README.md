...
- **Auto mode**: lamps always ON; incoming MQTT lux controls brightness
- **Manual override mode**: UI lux slider and per-lamp toggle buttons become active; MQTT lux/lamp messages are ignored
- `room/control/manual_override` is the only MQTT topic that can switch between modes
...
|---|---|---|
| Lamps | Always ON | Toggled per-lamp via UI |
...
| `room/control/manual_override` | `1` / `0` | Always accepted |
